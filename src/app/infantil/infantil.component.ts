...
// Sílabas ordenadas (para verificar la respuesta)
  ...
  fondos: string[] = [
    ...
    'https://i.pinimg.com/474x/fd/59/79/fd597991d63610aa85224a0d8cbe1427.jpg',
  ...
  elementoArrastrado: HTMLElement | null = null;

  onTouchStart(event: TouchEvent, silaba: string) {
    event.preventDefault();
    this.reproducirSonido(this.dragSound);

    this.silabaArrastrada = silaba;
    this.elementoArrastrado = event.target as HTMLElement;

    this.elementoArrastrado.classList.add('arrastrando');
  }
  ...
  onTouchMove(event: TouchEvent) {
    ...
    if (this.elementoArrastrado) {
      ...
      const offsetX = touch.clientX - this.elementoArrastrado.offsetWidth / 2;
      const offsetY = touch.clientY - this.elementoArrastrado.offsetHeight / 2;

      this.elementoArrastrado.style.position = 'fixed';
      this.elementoArrastrado.style.left = `${offsetX}px`;
      this.elementoArrastrado.style.top = `${offsetY}px`;
    }
  }

  onTouchEnd(event: TouchEvent, huecoIndex?: number) {
    event.preventDefault();

    if (this.silabaArrastrada && this.elementoArrastrado) {
      const touch = event.changedTouches[0];
      const touchX = touch.clientX;
      const touchY = touch.clientY;

      console.log('Posición del toque:', touchX, touchY);

      const huecoElement = this.findHuecoAtPosition(touchX, touchY);

      this.elementoArrastrado.style.position = '';
      this.elementoArrastrado.style.left = '';
      this.elementoArrastrado.style.top = '';
      this.elementoArrastrado.classList.remove('arrastrando');
      this.silabaArrastrada = null;
      this.elementoArrastrado = null;
    }
  }

  findHuecoAtPosition(x: number, y: number): HTMLElement | null {
    const huecos = document.querySelectorAll('.hueco');

    for (let i = 0; i < huecos.length; i++) {
      const hueco = huecos[i] as HTMLElement;
      const rect = hueco.getBoundingClientRect();

      console.log(`Hueco ${i}:`, rect);
      ...
      if (
        ...
        console.log(`Sílaba soltada sobre el hueco ${i}`);
        console.log('Silaba:', this.silabaArrastrada);
        //@ts-ignore
        this.huecos[i] = this.silabaArrastrada;
        this.reproducirSonido(this.dropSound);
        return hueco;
      }
    }
  ...
  verificarRespuesta() {
    const silabasCorrectas = this.silabasOrdenadas[this.palabraActualIndex];
    if (this.huecos.every((hueco, index) => hueco === silabasCorrectas[index])) {
      ...
      this.fondo = this.fondos[this.palabraActualIndex];
      this.puntos += 10;
